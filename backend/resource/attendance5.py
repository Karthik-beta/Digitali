    ...
    def _handle_out_log(self, employee: Employee, log: Logs) -> bool:
        ...
        try:
            ...
            if not attendance:
                ...
                attendance = Attendance.objects.filter(
                    Q(employeeid=employee) &  # Employee filter
                    Q(logdate=prev_date) &  # Date filter
                    Q(first_logtime__isnull=False)   # Must have a valid IN punch
                    # (Q(last_logtime__isnull=True) | Q(last_logtime__lt=F('first_logtime')))  # OUT punch is either null or before IN
                ).first()
                ...
                if not attendance:
                    ...
                    attendance = Attendance.objects.filter(
                        employeeid=employee,
                        logdate=log_date,
                        first_logtime__isnull=True
                    ).first()

                    if attendance.first_logtime is None:
                ...
                if attendance.last_logtime < attendance.first_logtime:
                    ...
                    try:
                        ...
                        # self.logger.error(f"Error fetching shift details for {attendance.shift}: {str(e)}")
                        return False
                    ...
                    # Update the last logtime if it's before the first logtime
